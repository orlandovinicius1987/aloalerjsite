...
class Contact implements Rule
{
    ...
    {
        $fromWorkflow = request('contact') == null;
        $pass = false;

        if ($fromWorkflow) {
            // workflow
            $pass = $this->regexRules($attribute, $value);
        } else {
            // InsertContats
            $contactTypeCode = app(ContactType::class)->find(
                request('contact_type_id')
            )->code;
            $pass = $this->regexRules($contactTypeCode, request('contact'));
        }
        return $pass;
    }
     ...
     * @return bool
     */
    private function regexRules($contactTypeCode, $contact)
    {
        $pass = false;
